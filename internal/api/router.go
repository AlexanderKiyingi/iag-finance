...
func NewRouter(deps RouterDeps) *gin.Engine {
	...
	v1 := router.Group("/v1", principal, middleware.EntityContext(), middleware.AuditLog(deps.AuditLog))
	{
		...
		v1.POST("/fixed-assets", ledgerWrite, api.RegisterFixedAsset)
		v1.POST("/fixed-assets/depreciation/run", ledgerWrite, api.RunDepreciation)
		...
		v1.POST("/fx/revalue", ledgerWrite, api.RevalueFX)
		v1.GET("/tax-codes", ledgerRead, api.ListTaxCodes)
		v1.POST("/tax-codes", ledgerWrite, api.UpsertTaxCode)
		v1.GET("/reports/vat-return", ledgerRead, api.VATReturn)
		v1.GET("/entities", ledgerRead, api.ListEntities)
		v1.POST("/entities", ledgerWrite, api.CreateEntity)
		v1.POST("/budgets", ledgerWrite, api.UpsertBudget)
		...
		v1.POST("/billing/invoices", ledgerWrite, api.CreateInvoice)
		v1.GET("/billing/invoices/:id", ledgerRead, api.GetBillingInvoice)
		v1.POST("/billing/invoices/:id/issue", ledgerWrite, api.IssueInvoice)
		v1.GET("/billing/recurring", ledgerRead, api.ListRecurringInvoices)
		v1.POST("/billing/recurring", ledgerWrite, api.CreateRecurringInvoice)
		v1.POST("/payments/intents", ledgerWrite, api.CreatePaymentIntent)
		v1.POST("/payments/intents/:id/confirm", ledgerWrite, api.ConfirmPaymentIntent)
		v1.GET("/projects", ledgerRead, api.ListProjects)
		v1.POST("/projects", ledgerWrite, api.CreateProject)
		v1.GET("/projects/:id/profit-and-loss", ledgerRead, api.ProjectPL)
		v1.GET("/cost-centers", ledgerRead, api.ListCostCenters)
		v1.POST("/cost-centers", ledgerWrite, api.CreateCostCenter)
